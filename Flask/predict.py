...
def auto_order_list(store_id):
    ...
    for product_id in db_result_df['product_id']:
        pred = pd.read_csv('data/pred_csv/pred_' + str(store_id) + '-' +
                           str(product_id) + '.csv')
        pred_sum_list.append(sum(pred['yhat'].iloc[21:]))
        pred_list.append(pred)

    db_result_df['pred_sum'] = pd.DataFrame(pred_sum_list,
                                            columns=['pred_sum'])
    db_result_df = db_result_df.sort_values(by='pred_sum',
    ...
    for pred in pred_top5_list:
        date_list.append(dict(date=pred['ds'].to_list()))
        pred_sale_list.append(dict(predict=pred['yhat'].to_list()))

    chart_data = dict(상품명=product_name_list, 날짜=date_list, 판매량=pred_sale_list)
    ...
    return {"chart_data": chart_data, "table_data": db_result_df_tolist}
